...
Navigate to the `start` directory and run the following command to build container images for your application:
[role='command']
```
...
| [hotspot=35]`cluster.ip`            | IP or host name for your cluster, `localhost` by default, which is appropriate when using Docker Desktop.

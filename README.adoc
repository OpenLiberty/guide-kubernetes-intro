...
The rollingUpdate configuration has two attributes within it which are [hotspot=maxUnavailable1 hotspot=maxUnavailable2]`maxUnavailable` and [hotspot=maxSurge1 hotspot=maxSurge2]`maxSurge`. maxUnavailable refers to the the number of Kubernetes pods that are allowed to be unavailable during the update process. Similarly, maxSurge refers to the number of additional pods that are allowed to be created during the update process.

The [hotspot=readinessProbe1 hotspot=readinessProbe2]`readiness check` for both Inventory and System has already been provided within the health directory of each. The remaining startup and liveness health checks can be found in the https://openliberty.io/guides/kubernetes-microprofile-health.html[Kubernetes-microprofile-health^] guide. 

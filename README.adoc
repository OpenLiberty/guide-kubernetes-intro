...
If you created a resource using the `kubctl create -f [FILENAME] options` command, then you can edit
the YAML describing the resource and reapply it using the `kubectl apply -f [FILENAME] options` command.

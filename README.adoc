...
a cluster as well as provides additional functionality. An `Ingress` requires an Ingress Controller 
to process requests accordingly.
...
instructions see https://github.com/kubernetes/minikube. Make sure to read the "Requirements" section 
as different operating systems require different prerequisites to get Minikube running.
...
If you chose to use Docker, please note that Docker for Windows uses Window's native Hyper-V installation, 
which tends to conflict with VirtualBox. As a result, you will might find it easier to run Minikube 
with Hyper-V rather than VirtualBox if you're on Windows. For more instructions on how to to do this, see 
...
When enabled, Minikube deploys two Pods in the `kube-system` namespaces. The first Pod, called `default-http-backend`, 
...
default-http-backend-59868b7dd6-v4qh7       1/1       Running   0          11m
nginx-ingress-controller-67956bf89d-6sg6r   1/1       Running   0          11m
----
...
Docker daemon and not your host's as described in the previous section.
...
To run integration tests against a cluster running at the `192.168.99.101` IP address:
...
Feel free to also stop your Minikube cluster:

```
minikube stop
```

And point the Docker daemon back to your local machine:

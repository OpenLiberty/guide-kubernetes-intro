...
:page-description: Explore how to deploy microservices to Kubernetes using Helm charts and manage them with the Kubernetes CLI kubectl.
...
Explore how to deploy microservices to Kubernetes using Helm charts and manage them with the Kubernetes CLI kubectl.
...
interface for {kube}. `kubectl` is your primary tool for communicating with and managing your {kube} cluster
...
enable as an addon in Minikube.
...
as this is what this guide will focus on. For installation instructions see https://docs.docker.com/install/
...
Note, if you're on Windows and you run into issues starting Minikube in VirtualBox, try using Hyper-V
...
To build your microservices and Docker image, you shouldn't need to start Docker locally as you've
...
Once the charts are installed, run the following command to check the status of your Pods:
...
free to to inspect all other resources created by the chart.
...
by default, so all you need to do is omit all Ingress-related parameters. For example, to install a
chart release for the `name` microservice without the Ingress, you would run the following command:

```
helm install --name ol-name-noingress --set image.pullPolicy=IfNotPresent --set image.repository=name --set image.tag=1.0-SNAPSHOT --set ssl.enabled=false --set service.port=9080 --set service.targetPort=9080 ibm-charts/ibm-open-liberty
```

Once the chart is installed, the {kube} Service choses a random `nodePort` within a predefined
range to expose. This is the port number through which you can access the Service encapsulating your
Pods. To find this port number, run the following command:

```
kubectl get services
```

The `nodePort` is the port number that the `9080` port maps to. In the following case the `nodePort`
is `31249`:
...
ol-name-noingress-ibm-open-liberty     NodePort    10.101.49.188    <none>        9080:31249/TCP   2s
----
...
Each unique session that you open to this URL will display a different Pod name, one for each of your
three running Pods. Notice also that no matter how many unique sessions you open, all your traffic will
be balanced perfectly among the three Pods. This is your Ingress controller's load balancer at work.
...
Next, navigate to the `name` directory withing the `start` directory and run the `mvn clean package`

...
:page-description: Explore how to deploy microservices to {kube} using Helm charts and manage them with kubectl.
...
= Deploying microservices to {kube}

Explore how to deploy microservices to {kube} using Helm charts and manage them with kubectl.
...
# From PowerShell if you're on Windows
minikube docker-env -u --shell=powershell dev | Invoke-Expression
```
...
name                                                             1.0-SNAPSHOT
open-liberty                                                     latest
gcr.io/{kube}-helm/tiller                                    v2.9.0
k8s.gcr.io/kube-proxy-amd64                                      v1.10.0
k8s.gcr.io/kube-controller-manager-amd64                         v1.10.0
k8s.gcr.io/kube-apiserver-amd64                                  v1.10.0
k8s.gcr.io/kube-scheduler-amd64                                  v1.10.0
quay.io/{kube}-ingress-controller/nginx-ingress-controller   0.12.0
...
k8s.gcr.io/{kube}-dashboard-amd64                            v1.8.1
...
[INFO] Local aggregation root: /Users/foo/Documents/repos/guides/wip/draft-guide-{kube}/finish
...
----
...
[INFO] --- exec-maven-plugin:1.6.0:exec (update-{kube}-deployment) @ name ---
...
----
{
  ...
  "metadata": {
    ...
    "annotations": {
      ...
      "nginx.ingress.{kube}.io/affinity": "cookie",
      "nginx.ingress.{kube}.io/rewrite-target": "/api/name",
      "nginx.ingress.{kube}.io/session-cookie-hash": "sha1",
      "nginx.ingress.{kube}.io/session-cookie-name": "route"
    }
  },
  "spec": {
    "rules": [
...
in the ready state, then run the following command:

```
mvn verify -Ddockerfile.skip=true
```

The `dockerfile.skip` parameter is set to `true` in order to skip building a new Docker image. If the
tests pass, you'll see an output similar to the following for each service respectively:
...
- `name.node.port` - Node port of the {kube} Service wrapping the `name` Pods.
- `ping.node.port` - Node port of the {kube} Service wrapping the `ping` Pods.

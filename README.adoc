...
will include building and containerizing the microservices, running the containers inside Kubernetes
Pods, setting up an Ingress to load balance traffic and persists sessions, establishing communicating
between the microservices, scaling Deployments, automatically rolling out new updates when the microservices
are rebuilt, editing Kubernetes resources, and testing microservices that are deployed to Kubernetes.
...
minikube docker-env --shell=powershell dev | Invoke-Expression
```
...
Whenever you make code changes and rebuild your Docker images, you'll need to update your Kubernetes
Deployments with your new image versions for the new code changes to be picked up.
...
Deployment in the existing `ol-name` release.

Installing a new chart release is done the same as before, using the `helm install` command. To update
an existing release, you need to update the image tag in the Deployment in order for it to point to
your new image version. To do this, run the following command:
...
of each microservice. This profile uses the Maven exec plugin to automatically run the
...
Please also note that you must install a chart release first before using the profile as there will
be nothing to update since your Deployment won't be created it. To invoke the profile when rebuilding,
use the `-P` flag followed by the profile name:
...
If you made an unstable Deployment update that resulted in breakage, you can revert the Deployment
back to a older revision by viewing its revision history:
...
To undo the current rollout and revert the Deployment back to its previous version, run the following command:
...
Kubernetes will terminate any existing Pods managed by the Deployment and create new ones from the
previous revision.
...
To make changes to Kubernetes resources, you can either edit that resource's YAML file in a text editor
or update particular parts of the YAML with the `kubectl` command.

=== Direct editing

Resource YAML files can be directly edited from the Kubernetes dashboard or a text editor of your choice.

To edit a resource from the Minikube dashboard, run `minikune dashboard` and then use the left
navigational panel to select which resources to edit.

To open and edit a resource in a text editor, run the `kubectl edit (RESOURCE/NAME | -f FILENAME) [options]`,
specifying the resource type and its name. Once you save your changes, they will be automatically picked
up and applied to your resource.

If you didn't create resources through charts, but rather implemented and created them yourself by
using the `kubectl create` command, then you can edit your YAML files directly and then apply
these changes by running the `kubectl apply -f [FILENAME] options` command.

To familiarize yourself with resource editing, edit the `ol-name-ibm-open-liberty` Ingress  and change
the `path` parameter from `/name` to `/myname`. Use any editing method you prefer:
...
When you're done editing, visit the new Ingress endpoint to verify that your changes have been successfully
applied to the Ingress.
...
to a newer version. `kubectl set` is convenient, but its limited to a small set of things that you can
change. Sometimes you might need to change parts of the resource that the `kubectl set` command simply
doesn't cover. In those cases and as an alternative to the `kubectl set` command, you can use the
`kubectl patch (-f FILENAME | TYPE NAME) -p PATCH [options]` command to update fields of a resource
using strategic merging. The `kubectl patch` command works by supplying a piece of config in a form
of a JSON or a YAML that matches another piece of similar config in the resource and overrides the
fields that don't match. For example, to change the Ingress path from `/name` to `/myname` as in the
...
Kubernetes will match your config pattern against the "spec" field defined in the `ol-name-ibm-open-liberty`
...
occurs, then you might have introduced a bug into the code. To run the tests, wait for all the Pods
to be in the ready state, then run the following command:

```
mvn verify -Ddockerfile.skip=true
```

The `dockerfile.skip` parameter is set to `true` to skip building a Docker image. If the tests pass,
you'll see a similar output for each service respectively:
...
Alternatively, if you didn't use an Ingress, you can use initialize the `name.node.port` and `ping.node.port`
properties like so to go through your Kubernetes Services instead:

```
mvn verify -Dname.node.port=<name-node-port> -Dping.node.port=<ping-node-port> -Ddockerfile.skip=true
```

The following properties are also available to be configured:
...
- `name.node.port` - Node port of the Kubernetes Service wrapping the `name` Pods.
- `ping.node.port` - Node port of the Kubernetes Service wrapping the `ping` Pods.
- `name.kube.service` - Name of the Kubernetes Service wrapping the `name` Pods, `ol-name-ibm-open-liberty` by default.
...
ran integration tests against miroservices that are running in a Kubernetes cluster.

...
[role="code_command hotspot", subs="quotes"]
----
#Update the Kubernetes configuration file in the `start` directory.#
`kubernetes.yaml`
----
...
The readiness check for both Inventory and System has already been provided within the health directory of each. The remaining startup and liveness health checks can be found in the Kubernetes-microprofile-health guide. 

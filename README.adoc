...
| [hotspot=34]`cluster.ip`            | IP or host name for your cluster, `localhost` by default, which is appropriate when using Docker Desktop.
| [hotspot=35]`system.kube.service`     | Name of the {kube} Service wrapping the `system` pods, `system-service` by default.
| [hotspot=36]`system.node.port`        | The NodePort of the {kube} Service `system-service`, 31000 by default.
| [hotspot=37]`inventory.node.port`        | The NodePort of the {kube} Service `inventory-service`, 32000 by default.
|===

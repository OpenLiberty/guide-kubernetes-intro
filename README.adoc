...
Your starting Java project, which you can find under the `start` directory, is a multi-module Maven
project thats made up of the `name` and `ping` microservices, each residing in their own directories,
`start/name` and `start/ping`. Each of these directories also contain a Dockerfile, which is necessary
for building Docker images. If you're unsure of what Dockerfiles are, check out our https://openliberty.io/guides/docker.html[Docker guide].
...
the microservices simultaneously through Maven. To do this, run the Maven `package` goal from the command line:
...
all Docker images and verify that the `name:1.0-SNAPSHOT` and `ping:1.0-SNAPSHOT` images are listed
among them.
...
check the Maven build log for any potential errors and make sure that your Docker CLI is configured
...
As mentioned previously, charts are sets of preconfigured Kubernetes resources, such as deployments,
services, ingresses, etc., all configured conveniently for some purpose. In this case, that purpose
is to run microservices in Open Liberty. All resources that are installed from a chart are configurable
just like any other Kubernetes resource. This allows you to tweak them to your liking or expand them
...
First, install an Open Liberty chart for the `name` microservice:
...
Both of these chart release will create three Kuberntes resource each: a deployment for managing pods,
a service for defining how these pods can be accessed, and an ingress for defining how external traffic
is routed to the Kubernetes service.

Each command is quite long and has a lot of flags, so let's break it down:
...
Similarly, you can also issue the `get` and `describe` commands on other Kubernetes resource. Feel
free to use them to inspect all other resources created by the chart.

Wait for the bods to be in the ready state, then access them via the ingress that you created earlier.
To do this, run the `minikube ip` command to get the IP address of your Minikube cluster, it will be
{minikube-ip} by default, and `curl` or visit the following URLs to access your microservices:
...
a good or bad response depending on whether the `kube-service` Kubernetes service is existent and accessible.

There is quite a bit going on when you make a request, so let's break it down. When you issue a request
to either URL, the NGINX ingress controller sees the request arrive at the apiserver's `/ingresses`
endpoint and re-routes this request appropriately using the set of rules you defined in your ingress
resource. This set of rules sates that all requests made to the https://{minikube-ip}/name/ URL are to
be mapped to the `/api/name` endpoint of the Kubernetes service running `name` pods. Similarly for
the https://{minikube-ip}/ping/ URL. Once the request arrives at each Kubernetes service, it uses its
own set of rules to map this request to a pod, which then sends back a response, which the Kubernetes
service passes back to the client.

=== Alternative: deploying microservice without ingress

You can choose to omit the ingress resource and simply install your charts without it. To do this,
simply omit all ingress-related parameter as ingress is disable by default.

Once your chart is installed, a `nodePort` will be exposed by the Kubernetes service. This is the
port by which you can access the Kubernetes service and hence the pods inside it. This port is assigned
randomly without a predefined range and you can get it by either running one of the commands listed
at the end of the chart install or by running the `kubectl get services` command. The `9080` port
will be mapped to an external `nodePort`:
...
Then access your microservice by `curling` or visiting http://{minikube-ip}:31249/api/name (in this case).

The disadvantage to this approach is that Kubernetes doesn't have a practical load balancer. In fact,
traffic is balanced randomly by default. This could prove extremely inconvenient when, by chance, all
traffic is routed to a single pod among dozens, overloading it and causing it to crash.
...
instances of your microservices. Scaling is one of the primary function of Kubernetes as it allows you
to scale your applications whenever you want to accommodate for more traffic, and descale whenever to
free up resource when traffic is lower.

As an example, scale the `name` deployment to have 3 running pods instead of just 1. This can be done
...
Next, run `kubectl get pods` and wait until your two new pods are in the ready state:
...
how many unique sessions you open to this URL, all your traffic will be balanced perfectly among the
three pods. This is your NGINX ingress controller load balancing traffic.

Note, that if you attempt to open a session in a new browser tab, you will end up with the same pod.
This is due to sticky sessions, meaning that Kubernetes is passing your requests to the same backend
container each time in order to persistent any data you might have created during your first session.
Sticky sessions can be configured and disabled from your ingress resource.


// =================================================================================================
// Editing resources and rolling out deployment updates
// =================================================================================================

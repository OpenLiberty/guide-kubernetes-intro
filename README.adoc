...
:common-includes: https://raw.githubusercontent.com/OpenLiberty/guides-common/maven-gradle
...
If you don't see the `system:1.0-SNAPSHOT` and `inventory:1.0-SNAPSHOT` images, then check the 
...
from the **system-service** endpoint to the inventory {kube} Service. 
Making a request to the `http://$INVENTORY_PROXY/inventory/systems/[kube-service]` URL in general 
adds to the inventory. That result depends on whether the **kube-service** endpoint is a valid {kube} service 
...
[source, role="maven_section command"]
----
./mvnw failsafe:integration-test -Dcluster.ip=$(minikube ip)
----

[source, role="gradle_section command"]
----
./gradlew test -Dcluster.ip=$(minikube ip)
----
--
endif::[]

...
To edit from a resource from the Minikube dashboard, run `minikune dashboard` and then use the left
...
change. Sometimes you might simply need to change parts of the resource that the `kubectl set` command
...
fields that don't match. For example, change the Ingress path from `/name` to `/myname` as in the previous

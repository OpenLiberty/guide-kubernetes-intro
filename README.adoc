...
You will learn how to deploy two microservices in Open Liberty Docker containres to a local {kube} cluster using the Kubernetes package
...
You will use a local single-node {kube} cluster and employing NGINX as your Ingress controller.
...
- `kubernetes` - the {kube} cluster. If you have a Docker installation that provides a local {kube} cluster then just use that. For example, in https://docs.docker.com/docker-for-windows/[Docker for Windows] a local {kube} environment is enabled through https://docs.docker.com/docker-for-windows/#kubernetes[settings]. Otherwise you can use `Minikube` as a single-node {kube} cluster that runs locally in a virtual machine. For Minikube installation instructions see https://github.com/kubernetes/minikube. Make sure to read the "Requirements" section as different operating systems require different prerequisites to get Minikube running. 
- `kubectl` - a command line client interface for {kube}. If you have a Docker installation that provides kubectl, such as https://docs.docker.com/docker-for-windows/[Docker for Windows] then just use that. Otherwise install the client as described at https://kubernetes.io/docs/tasks/tools/install-kubectl/
...
kubectl get ingress
```
The default Ingress hostname for minikube is {minikube-id}.  
...
{kube} Service. Visiting https://[ingress-ip]/ping/{kube-service} in general will return either
...
| cluster.ip        | IP address of your Igress, `{minikube-ip}` by default.
...
To run integration tests against a cluster running at `localhost`:
...
To use the {kube} dashboard to edit a resources you first need to install and start the dashboard. If you are using minikube then simply run the `minikube dashboard` command to open the dashboard. Otherwise install the {kube} dashboard as described at https://kubernetes.io/docs/tasks/access-application-cluster/web-ui-dashboard/ . Once you have accessed the dashboard in a browser, use the left navigational panel to select which resources to edit.

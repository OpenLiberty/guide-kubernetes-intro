...
The two microservices you will deploy are called `system` and `inventory`. The `system` microservice simply
displays a brief greeting and the name of the
container that it runs in, making it easy to distinguish it from its other replicas. The `inventory` microservice
...
This file defines four {kube} resources. It defines two deployments and two services. A {kube} deployment is a resource responsible for controlling the creation and management of pods. A service exposes your deployment so that you can make requests to your containers. Three key items to look at when creating the deployments are the [hotspot=5 hotspot=26 hotspot=34]`label`, [hotspot=18 hotspot=39]`image`, and [hotspot=20 hotspot=41]`containerPort` fields. The [hotspot=5 hotspot=26 hotspot=34]`label` is a way for a {kube} service to reference specific deployments. The [hotspot=18 hotspot=39]`image` is the name and tag of the docker image that you want to use for this container. Finally, the [hotspot=20 hotspot=41]`containerPort` is the port that your container exposes for purposes of accessing your application. 
For the services, the key point to understand is that they expose your deployments. 
The binding between deployments and services is specified by the use of labels -- in this case the [hotspot=6 hotspot=10 hotspot=14 hotspot=27 hotspot=31 hotspot=35 hotspot=50 hotspot=64]`app` label. 
You will also notice the service has a type of [hotspot=48 hotspot=62]`NodePort`. 
This means you can access these services from outside of your cluster via a specific port. 
In this case, the ports will be `31000` and `32000`, but it can also be randomized if the [hotspot=55 hotspot=69]`nodePort` field is not used.
...
The second URL adds proprties from `system-service` to the inventory.
...
As an example, scale the `system` Deployment to 3 pods by running the following command:
[role='command']
```
...
| [hotspot=36]`name.kube.service`     | Name of the {kube} Service wrapping the `name` pods, `name-service` by default.
| [hotspot=37]`name.node.port`        | The NodePort of the {kube} Service `name-service`, 31000 by default.
| [hotspot=38]`ping.node.port`        | The NodePort of the {kube} Service `ping-service`, 32000 by default.
|===
...
You can also run the integration tests with an IP address of [hotspot=35]`192.168.99.100`:
[role='command']
```
...
Tests run: 2, Failures: 0, Errors: 0, Skipped: 0, Time elapsed: 0.567 s - in it.io.openliberty.guides.system.SystemEndpointTest
...
Tests run: 1, Failures: 0, Errors: 0, Skipped: 0, Time elapsed: 0.893 s - in it.io.openliberty.guides.inventory.InventoryEndpointTest

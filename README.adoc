...
returns the JVM systems properties of the container it is running in and it returns the Pod's name in the HTTP header
making it easy to distinguish it from its other replicas. The `inventory` microservice
...
| [hotspot=35]`cluster.ip`            | IP or hostname for your cluster, `localhost` by default, which is appropriate when using Docker Desktop.

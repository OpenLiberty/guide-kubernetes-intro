...
{kube} is a primary tool for deployment in containerized environments. 
...
A {kube} deployment is a resource responsible for controlling the creation and management of pods. 
A service exposes your deployment so that you can make requests to your containers. 
Three key items to look at when creating the deployments are the [hotspot=labels1 hotspot=labels2 hotspot=labels3 hotspot=labels4]`labels`, [hotspot=image1 hotspot=image2]`image`, and [hotspot=containerPort1 hotspot=containerPort2]`containerPort` fields. 
...
Finally, the [hotspot=containerPort1 hotspot=containerPort2]`containerPort` is the port that your container exposes for purposes of accessing your application.
...
Then use `curl` or visit the following URLs to access your microservices, substituting the appropriate host name:
...
Scaling is one of the primary advantages of {kube} because replicating your application allows it to accommodate more traffic, and then descale your deployments to free up resources when the traffic decreases.
...
Wait for your two new pods to be in the ready state, then `curl -I` or visit the `{system-api}` URL. 
...
To do that, you can rebuild your Docker images then delete and re-create your {kube} resources. 
...
This is not how you would want to update your applications when running in production, but in a development environment this is fine. 
If you want to deploy an updated image to a production cluster, you can update the container in your deployment with a new image. 
Then, {kube} will automate the creation of a new container and decommissioning of the old one once the new container is ready.
...
| [hotspot=cluster.ip]`cluster.ip`            | IP or host name for your cluster, `localhost` by default, which is appropriate when using Docker Desktop.
...
| [hotspot=system.node.port]`system.node.port`        | The NodePort of the {kube} Service `system-service`, 31000 by default.
| [hotspot=inventory.node.port]`inventory.node.port`        | The NodePort of the {kube} Service `inventory-service`, 32000 by default.
|===
...
You have just deployed two microservices running in Open Liberty to {kube}. 
